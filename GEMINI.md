...
- **Gumroad license-key generation is a dashboard-only setting.** The product must have "Generate a unique license key per sale" enabled or buyers receive no key and the app stays gated. The storefront payload exposes it as `is_licensed`; check with `curl -sL https://olivera40.gumroad.com/l/pingwarden | grep -o 'is_licensed&quot;:[a-z]*'`. Neither the public API nor the `gumroad` CLI can toggle it: `licenses verify/enable/disable/rotate` manage existing keys only. Never publish the product with `is_licensed:false`.
